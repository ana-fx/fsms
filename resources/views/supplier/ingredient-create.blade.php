                    ...
                    <form action="{{ route('supplier.ingredients.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="food_category_id" id="food_category_id" required disabled class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent bg-gray-100">
                            ...
                            <div>
                                ...
                                <input type="text" name="sub_name" id="sub_name" value="{{ old('sub_name') }}" class="w-full px-4 py-2 border {{ $errors->has('sub_name') ? 'border-red-500' : 'border-gray-300' }} rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="e.g., Merah, Premium, 2 KARUNG">
                                <p class="mt-1 text-xs text-gray-500">
                                    <i class="fas fa-info-circle mr-1"></i>
                            ...
                            <div>
                                ...
                                <div id="name_preview" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-600">
                                    <span class="text-sm">Name will appear here...</span>
                                </div>
                                <p class="mt-1 text-xs text-gray-500">
                                    <i class="fas fa-info-circle mr-1"></i>
                            ...
                            <div>
                                ...
                                <div id="maxPriceInfo" class="hidden mt-1">
                                    <p class="text-xs text-blue-600">
                                        <i class="fas fa-info-circle mr-1"></i>
                                        Maximum price limit for category "<span id="categoryName"></span>": <strong id="maxPriceValue"></strong> (set by administrator)
                                    </p>
                                </div>
                                @error('price')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Max price info elements
    const maxPriceInfo = document.getElementById('maxPriceInfo');
    const categoryNameEl = document.getElementById('categoryName');
    const maxPriceValueEl = document.getElementById('maxPriceValue');
    
    // Current max price from selected category
    ...
    function updateItemNameDropdown() {
        ...
        if (parentId && subCategories.length > 0) {
            // Enable dropdown and remove disabled state
            itemNameSelect.disabled = false;
            itemNameSelect.classList.remove('bg-gray-100');
            itemNameSelect.classList.add('bg-white');
            ...
            // Disable dropdown if no parent selected or no sub-categories
            itemNameSelect.disabled = true;
            itemNameSelect.classList.add('bg-gray-100');
            itemNameSelect.classList.remove('bg-white');
        }
    ...
    function updateNamePreview() {
        ...
        // Update max_price from selected category
    ...
    function updateMaxPriceFromCategory() {
        ...
        if (selectedCategoryId) {
            ...
            for (const parentId in subCategoriesByParent) {
                ...
                if (selectedCategory && selectedCategory.max_price) {
                    ...
                    // Show max price info
                    categoryNameEl.textContent = selectedCategory.name;
                    maxPriceValueEl.textContent = 'Rp ' + currentMaxPrice.toLocaleString('id-ID');
                    maxPriceInfo.classList.remove('hidden');
                    // Update validation
                    validatePriceLimit();
                    return;
                }
        ...
        maxPriceInfo.classList.add('hidden');
        validatePriceLimit();
    }
    ...
    // Initialize on page load if parent category is already selected (from old input)
    if (parentCategorySelect && parentCategorySelect.value) {
        updateItemNameDropdown();
        // Restore old selected value if exists
        const oldItemId = '{{ old("food_category_id") }}';
        if (oldItemId) {
            itemNameSelect.value = oldItemId;
            updateNamePreview();
        }
    }
    ...
    // Add event listeners for max purchase and price validation
    if (priceInput) {
    ...
    function closeAutoCorrectModal(accept) {
        ...
        if (modal) {
            ...
            if (accept && priceInput && currentMaxPrice > 0) {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            let message = `Ingredient will be created with:<br>` +
            ...
            if (confirmMaxPrice > 0) {
                message += `• Maximum Price Limit for "${itemName}": <strong>Rp ${confirmMaxPrice.toLocaleString('id-ID')}</strong> (set by admin)<br>`;
            }
